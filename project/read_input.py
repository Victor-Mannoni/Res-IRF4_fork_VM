...
def read_inputs(config, other_inputs=generic_input):
    ...
    cost_insulation = get_pandas(config['technical']['cost_insulation'], lambda x: pd.read_csv(x, index_col=[0]).rename_axis('Year').rename_axis('Insulation', axis=1))
...
def parse_inputs(inputs, taxes, config, stock):
    ...
    c_insulation = inputs['cost_insulation'].reindex(idx_years, method='ffill')
    parsed_inputs.update({'cost_insulation': c_insulation})
